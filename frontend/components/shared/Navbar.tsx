...
export default function Navbar() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 px-3 pt-3 sm:px-4 sm:pt-4">
      <div className="max-w-6xl mx-auto flex items-center justify-between px-3 py-2 sm:px-4 sm:py-2.5 rounded-2xl bg-white/80 backdrop-blur-md border border-white/60 shadow-[0_4px_24px_rgba(0,0,0,0.06)]">
        ...
        {/* CENTER: Desktop nav links (ADMIN only) */}
        {navLinks.length > 0 && (
          <nav className="hidden md:flex items-center gap-1 absolute left-1/2 -translate-x-1/2">
            {navLinks.map((link) => (
              <Link
                ...
                className={
                  isActive(link.href)
                    ? 'px-4 py-2 rounded-xl text-sm font-medium text-gray-900 bg-gray-100 transition-all duration-200'
                    : 'px-4 py-2 rounded-xl text-sm font-medium text-gray-500 hover:text-gray-900 hover:bg-gray-100/80 transition-all duration-200'
                }
              >
                {link.label}
              </Link>
        ...
        {/* RIGHT: Mobile sheet + desktop dropdown */}
        <div className="flex items-center gap-2 flex-shrink-0">

          {/* Mobile hamburger using Sheet */}
          <Sheet>
            <SheetTrigger className="md:hidden p-2 rounded-xl hover:bg-gray-100/80 transition-colors duration-200 text-gray-600">
            ...
            <SheetContent side="left" className="w-72 p-0 bg-white border-r border-gray-100">
              ...
              <div className="flex items-center gap-3 px-6 py-5 border-b border-gray-100">
                <LogoIcon />
                <span className="font-bold text-gray-900 text-base">TaskFlow</span>
              </div>
              ...
              <div className="px-4 py-4 border-b border-gray-100">
                <div className="flex items-center gap-3 p-3 rounded-2xl bg-gray-50">
                  ...
                  <div className="min-w-0">
                    ...
                    <Badge variant="outline" className="mt-1 text-xs rounded-full bg-violet-50 text-violet-600 border-violet-200">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-100">
                <SheetClose
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-red-500 hover:bg-red-50 hover:text-red-600 transition-colors duration-150"
                >
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              style={gradientStyle}
              className="hidden md:flex items-center gap-2 px-3 py-2 rounded-xl text-white text-sm font-medium hover:opacity-90 active:scale-95 transition-all duration-200 outline-none focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-400"
            >
            ...
            <DropdownMenuContent
              align="end"
              sideOffset={8}
              className="w-60 rounded-2xl p-0 overflow-hidden bg-white/90 backdrop-blur-md border border-gray-100 shadow-[0_8px_32px_rgba(0,0,0,0.12)]"
            >
              {/* User info header */}
              <div className="px-4 py-3 border-b border-gray-100">
                ...
                <div className="mt-2.5">
                  <Badge variant="outline" className="text-xs rounded-full bg-violet-50 text-violet-600 border-violet-200 font-medium">
              ...
              <div className="p-1.5">
                <DropdownMenuItem
                  onClick={handleLogout}
                  className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-medium text-red-500 hover:text-red-600 hover:bg-red-50 focus:bg-red-50 focus:text-red-600 cursor-pointer transition-colors duration-150"
                >
